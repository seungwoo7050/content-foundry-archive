...
A release is wholly `2.0.0`, wholly `3.0.0`, or wholly `4.0.0`; records from
different versions must not be mixed. Version dispatch happens before record
validation or rendering. Consumers must not silently downgrade or reinterpret
v4 as v3/v2, or v3 as v2. Supporting the v4 shape without its presentation
reference and readiness semantics does not count as `4.0.0` support.

Contract promotion does not claim that either implementation supports v4.
ContentOps may emit `4.0.0` only when its producer is complete and the target
Public Sites deployment declares complete `4.0.0` support. Existing v2/v3
release and rollback behavior remains unchanged.
